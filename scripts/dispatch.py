...
_SIMPLE_HANDLERS = [
    ...
    (re.compile(r'^給我建議|^今日忠告|^人生建議|^今日建議|^金玉良言'), _cmd_advice),
    (re.compile(r'笑話|冷笑話'), _cmd_joke),
    (re.compile(r'冷知識|告訴我一件事'), _cmd_fun_fact),
    (re.compile(r'^今日調酒|^調酒|^雞湯'), _cmd_cocktail),
    (re.compile(r'^今日食譜|^隨機食譜|^吃什麼食譜|^今天吃什麼|^晚餐吃什麼|^午餐吃什麼|^吃什麼$'), _cmd_meal),
    (re.compile(r'^今日運動|^找運動|^來運動|^來健身|^運動建議|^健身建議'), _cmd_exercise),
    (re.compile(r'動漫語錄|二次元語錄|動漫名言|今日動漫'), _cmd_anime_quote),
    (re.compile(r'川普語錄|美國總統語錄|總統語錄'), _cmd_trump_quote),
    (re.compile(r'梗圖|隨機梗圖'), _cmd_meme),
    (re.compile(r'諾里斯|chuck|norris|查克諾里斯|功夫笑話'), _cmd_chuck_norris),
    (re.compile(r'名人語錄|激勵名言|今日名言|^名言$|給我力量'), _cmd_quotable),
    ...
    (re.compile(r'^新聞|^今日新聞|^最新新聞|^頭條|^今天新聞'), _cmd_news),
    (re.compile(r'^無聊|^我好無聊|^打發時間|^今天做什麼|^推薦活動|^隨機活動'), _cmd_activity),
...
_DAILY_HANDLERS = [
    (re.compile(r'^今日日文單字|^日文單字|^學日文|^每日日語|^今日日文'), _cmd_daily_japanese),
    (re.compile(r'^今日漢字|^漢字|^學漢字|^每日漢字'), _cmd_daily_kanji),
    (re.compile(r'^今日西文單字|^西文單字|^學西文|^學西班牙文|^每日西語|^今日西文'), _cmd_daily_spanish),
    (re.compile(r'電影台詞|電影名言|名片台詞'), _cmd_movie_quote),
